...
CTexture_MD2Manager	*CTexture_MD2Manager::m_singleton = 0;



// ----------------------------------------------
// GetInstance() - get a pointer to the unique
// CTexture_MD2Manager instance (the singleton)
// ----------------------------------------------

CTexture_MD2Manager *CTexture_MD2Manager::GetInstance(void)
{
	if (m_singleton == 0)
		m_singleton = new CTexture_MD2Manager;

	return ((CTexture_MD2Manager *)m_singleton);
}



// ----------------------------------------------
// FreeInstance() - delete the unique CTexture_MD2Manager
// instance.
// ----------------------------------------------

void CTexture_MD2Manager::FreeInstance(void)
{
	if (m_singleton != 0)
	{
		delete m_singleton;
...
void CTexture_MD2Manager::Initialize(void)
{
	...
	{
	CTexture_MD2 *tex = new CTexture_MD2("default");
...
unsigned int CTexture_MD2Manager::LoadTexture(const char *filename)
{
	unsigned int	id = 0;
	...
	// create a new CTexture_MD2 object and push it at the end of the linked list
	CTexture_MD2 *tex = new CTexture_MD2(id, filename);
	m_texlist.push_back(tex);
...
void CTexture_MD2Manager::DeleteTexture(unsigned int id)
{
	for (TListItor itor = m_texlist.begin(); itor != m_texlist.end(); ++itor)
	{
		if ((*itor)->GetTexId() == id)
...
void CTexture_MD2Manager::CleanAllTextures(void)
{
	for (TListItor itor = m_texlist.begin(); itor != m_texlist.end(); ++itor)
	{
		delete (*itor);
